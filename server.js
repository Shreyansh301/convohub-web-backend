...
io.on("connection", async (socket) => {
  ...
  // Listen for "chat_message" events from clients (Create operation)
  socket.on("chat_message", async (msg) => {
    // Save the message to Supabase (Insert into database)
    const { error: insertError } = await supabase
      .from("messages")
      .insert([{ name: "User", message: msg }]); // Insert the message with a default name "User"
  ...
  // Listen for "edit_message" events from clients (Update operation)
  socket.on("edit_message", async ({ msgId, updatedMessage }) => {
    // Update the message in Supabase (Update operation)
    const { error: updateError } = await supabase
      .from("messages")
      .update({ message: updatedMessage })
      .eq("id", msgId); // Update message by ID

    // Handle errors when updating the message
    if (updateError) {
      console.error("Error editing message:", updateError.message);
    } else {
      // Broadcast the updated message to all clients
      io.emit("message_edited", { msgId, updatedMessage });
    }
  });

  // Listen for "delete_message" events from clients (Delete operation)
  socket.on("delete_message", async (msgId) => {
    // Delete the message from Supabase (Delete operation)
    const { error: deleteError } = await supabase
      .from("messages")
      .delete()
      .eq("id", msgId); // Delete message by ID

    // Handle errors when deleting the message
    if (deleteError) {
